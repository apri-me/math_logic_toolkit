...
def truth_function_generator(formula: str, var_names: tuple[str], connectives_dict: dict):
...
formula is a well-formed formula in a string data and var_names is a set of variable names that may be used in formula."""
    formula1 = formula.strip()[1:-1].strip()
    ...
    con = tape_cons[0]
    tape.remove(con[1])
    tape_funcs = [truth_function_generator(a, var_names, connectives_dict) for a in tape]
...
def main():
    ...
    f = truth_function_generator(formula3, vars, connectives_dict)
    print(f(0, 0))
